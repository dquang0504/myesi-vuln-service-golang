...
func (o *AlembicVersion) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(alembicVersionAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(alembicVersionPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert alembic_version, could not build conflict column list")
			}

			conflict = make([]string, len(alembicVersionPrimaryKeyColumns))
			copy(conflict, alembicVersionPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"alembic_version\"", updateOnConflict, ret, update, conflict, insert, opts...)
